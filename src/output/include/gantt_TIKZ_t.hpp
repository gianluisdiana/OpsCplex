...
public:
    ...
    std::ostream& full_description(std::ostream& os, const OPS_output_t& O) const
    {


        os << "\t \t \\begin{tabular}{ llcr }" << '\n';
        os << "\\\\" << '\n';
        os << "\t \t  $|V^*|$ & \\hspace{3.3cm} &  &" << std::setw(8) << O.n_customers() << " \\\\" << '\n';
        os << "\t \t  max length                &  &  &" << std::setw(8) << O.length() << " \\\\" << '\n';
        os << "\t \t  Obj             &  &  &" << std::setw(8) << std::fixed << std::setprecision(2) <<  O.get_obj() << " \\\\" << '\n';
        //os << "\t \t  CSU$_h$            &  &  &" << std::setw(6) << std::fixed << std::setprecision(2) << CSU_st.get_CSU_h() << " \\\\" << '\n';
        //os << "\t \t  $h$                &  &  &" << std::setw(6) << std::fixed << std::setprecision(2) << CSU_st.get_h() << " \\\\" << '\n';
        //os << "\t \t  $d_1$               & &  &" << std::setw(6) << std::fixed << std::setprecision(2) << CSU_st.get_d1() << " \\\\" << '\n';
        //os << "\t \t  $d_2$               & &  &" << std::setw(6) << std::fixed << std::setprecision(2) << CSU_st.get_d2() << " \\\\" << '\n';
        os << "\t \t \\end{tabular}" << '\n';
        os << '\n';
        return os;
    }
    ...
    void g_setup(std::ostream& os, const OPS_output_t& O, int k, int i, int j) const
    {
        const int    n    = I_.get_n();
        const int    K    = I_.get_m();
